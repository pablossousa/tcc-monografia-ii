...
\newcommand{\requirements}{\textit{biblatex 3.8} e \textit{biber 2.8}}
...
O \textit{biblatex-abnt \version} requer \requirements. Caso haja algum
problema na compilação, cheque se seus pacotes estão atualizados.
% <<<2

\section{Instalação}% >>>2

O \textit{biblatex-abnt} está incluso no TeX Live a partir 2016.

Para instalá-lo manualmente, copie os arquivos \textit{.bbx}, \textit{.cbx} e
\textit{.lbx} para
\begin{verbatim}
...
Após a opção \textit{style}, podem ser acrescentadas
...
As opções a seguir podem ser usadas ao chamar o \textit{biblatex} (além
...
\begin{description}[itemindent=-1em,leftmargin=3em]% >>>3
  ...
  \item [dateyear] Imprime apenas o ano das entradas, descartando todas as outras partes da data. Se utilizada como uma opção global, recomenda-se o uso da opção \textit{date=year} em seu lugar (menor risco de bugs)
  ...
  \item [usedashes] Usa os traços padrão do \textit{biblatex} nos campos
    repetidos
  ...
  \item [language=brazil] Essa opção é adicionada automaticamente. Para
    ...
    \textit{brazil} pelo código da linguagem desejada
...
\begin{sloppypar}
  As opções \textit{repeatfields}, \textit{repeattitles}, \textit{backref},
  \textit{nosl}, \textit{nosn}, \textit{noslsn}, \textit{extrayear}, \textit{dateyear}, \textit{datemonth} e \textit{slashdaterange} também
...
Ou, um exemplo de uso do \textit{slashdaterange}:
\begin{verbatim}
    @article{lucca2009,
...
As opções \textit{dateyear} e \textit{datemonth} também podem ser aplicadas apenas para tipos específicos de entradas, através do comando \verb|\ExecuteBibliographyOptions|. E. g.:
...
\subsection{Estilo \textit{abnt}}% >>>3
...
\begingroup
\let\clearpage\relax
\subsection{Estilo \textit{abnt-numeric}}% >>>3
\endgroup
...
\begingroup
\let\clearpage\relax
\subsection{Estilo \textit{abnt-ibid}}% >>>3
\endgroup

No estilo \textit{abnt-ibid}, o comando \textit{footcite} imprime, conforme as
instruções da ABNT, a referência inteira, sua versão abreviada ou as
abreviações ``id.'', ``ibid.'', ``op. cit.'' e ``loc. cit.''. O comando
\textit{apud} também é adaptado para usar as notas de rodapé.
...
\textit{biblatex}. Estes são alguns exemplos de situações comuns:
...
\noindent
Também é possível usar o campo \textit{crossref} para herdar as informações de
outra entrada:
...
% \clearpage
% \section{Compatibilidade com o pacote \textit{abntex2cite}}% >>>2

% Em geral é possível usar o mesmo arquivo \textit{.bib} utilizado pelo
% \textit{abntex2cite}, e para a maior parte das entradas mais simples nenhuma
% mudança é necessária; estas são as principais exceções:

% \begin{itemize}
%   \begin{sloppypar}
%   \item Não consegui entender por que, mas os exemplos do
%     \textit{abntex2cite} usam \verb"\'\i" para o caractere ``í'', enquanto o
%     normal me parece ser \verb"\'i" (para outras letras com acento agudo o
%     \textit{abntex2cite} usa o formato normal). Isso pode causar alguns
...
%     motivo, chamar o biblatex com a opção \textit{safeinputenc} pode
...
%   \item Os campos de datas (\textit{date}, \textit{year}, \textit{month} etc.)
%     devem seguir o formato usado pelo \textit{biblatex} (\textit{yyyy-mm-dd}).
%     Cf. seção 2.2.1 do manual.
%   \item O \textit{biblatex} diferencia os campos \textit{pages} (e.g. ``p.
%     12-18'') e \textit{pagetotal} (e.g. ``347 p.'').
...
%   \item Pode-se incluir no campo \textit{options} as opções \textit{nosl}
%     (para não mostrar a abreviação ``[s.l]''), \textit{nosn} (para não
%     mostrar ``[s.n.]'') ou \textit{noslsn} (para não mostrar nenhuma das duas
%     abreviações) (e.g. \textit{options = \{noslsn\}}); essa configuração
%     tem efeito em cada entrada específica. Pode-se também incluir essas
%     opções
%     ao chamar o \textit{biblatex} para nunca mostrar as abreviações, e
%     então abrir exceções para entradas específicas (e.g. \textit{options =
%     \{noslsn=false\}}).
%   \item Para periódicos, deve se utilizar entradas \textit{@periodical}.
%     Usando-se \textit{@book}, como no \textit{abntex2cite}, o ISSN não
%     aparece.
%   \item Para teses de mestrado e doutorado, o campo \textit{pagetotal} é
%     automaticamente formatado em folhas, como requer a norma (e.g. ``347
%     f.''). Para usar páginas também nessas entradas, use
%     \textit{bookpagination =
%     \{page\}}. Para usar folhas em outras entradas, use
%     \textit{bookpagination = \{sheet\}}.
%   \item Ao modificar o campo \textit{type} em teses de mestrado e doutorado,
%     deve-se preenchê-lo com todo o texto desejado (e.g. ``Tese (Doutorado em
%     Nutrição)'').
%   \item Ao utilizar o campo \textit{illustrated}, deve-se preenchê-lo com
%     todo o texto desejado (e.g. ``il.'').
%   \item Além do campo \textit{organization}, há o campo \textit{nameaddon},
...
%   \item O \textit{abntex2cite} às vezes usa o campo \textit{type} como um
%     complemento da organização (um exemplo é a entrada
%     \textit{brasil1988}).
%     Com o \textit{biblatex-abnt} deve-se usar o campo \textit{nameaddon} em
%     vez de \textit{type}. Embora nesse caso (``Constituição (1988)'') o
%     campo
%     \textit{type} faça mais sentido semanticamente, colocá-lo nessa
%     posição causaria problemas na organização alfabética da bibliografia,
%     já que o campo \textit{type} às vezes aparece antes do título e às
%     vezes depois.
%   \item A recomendação do manual do \textit{biblatex} (seção 2.3.3) é de
%     que, para autores corporativos, utilize-se os campos \textit{author} e
%     \textit{editor}, colocando o nome da organização entre chaves. Essa
%     opção tem a vantagem de permitir que se misture autores corporativos e
%     autores comuns (e.g. \textit{editor = \{National Aeronautics and Space
%     Administration\} and John Doe}).
%   \item Quando o nome do autor, editor ou organização for muito grande para
%     usar nas citações, pode-se acrescentar os campos \textit{short\-author}
%     e \textit{short\-editor} (e.g. \textit{author = \{National Aeronautics and
%     Space Ad\-min\-is\-tra\-tion\},} \textit{short\-author = \{NASA\}}
%     imprimirá ``NASA'' nas citações e ``National Aeronautics and Space
%     Administration'' na bibliografia). O campo \textit{org-short}, usado pelo
%     \textit{abntex\-2\-cite}, é automaticamente convertido para
%     \textit{shortauthor}.
%   \item O separador ``de'' faz com que tudo o que vem depois dele seja
%     considerado um único sobrenome (e.g. na entrada \textit{alves1995} o nome
%     \textit{Roque de Brito} \textit{Alves} é impresso como ``BRITO ALVES,
%     Roque de''). Cf. \url{http://tex.stackexchange.com/q/308625/102699}.
%   \item Quando os campos \textit{number}, \textit{volume}, \textit{chapter} e
%     \textit{edition} contêm apenas números, eles são formatados
%     automaticamente (e.g. \textit{edition = \{5\}} imprime ``5. ed.''). Quando
...
%     \textit{.,-/} podem ser usados e o campo ainda será considerado como
%     contendo apenas números.
%   \item Em entradas dos tipos \textit{phdthesis}, \textit{mastersthesis} e
%     \textit{monography}, há a data de publicação, que aparece logo após o
%     título, e a data da defesa, que aparece por último. O
%     \textit{abntex2cite}
%     às vezes usa o campo \textit{year-presented} para diferenciar entre as
%     duas
%     datas e às vezes muda as opções do pacote para mostrar uma mesma data
%     em
%     uma ou outra posição. No \textit{biblatex-abnt} pode-se usar os campos
%     de
%     datas usuais para a data de publicação, que aparece após o título, e
%     os campos \textit{eventdate}, \textit{eventmonth} e \textit{eventyear}
%     para a
%     data da defesa, que aparece no final. O campo \textit{year-presented},
%     usado pelo \textit{abntex2cite}, é automaticamente convertido para
%     \textit{eventyear}. (Cf. entradas \textit{morgado1990},
%     \textit{morgadob1990} e \textit{morgadoc1990} no arquivo
%     \textit{abnt-testcase.tex}.)
%   \item Em vez dos campos \textit{reprinted-from} e \textit{reprinted-text},
%     utilizados pelo \textit{abntex2cite}, usa-se o campo \textit{related} para
%     citar uma entrada relacionada e o campo \textit{relatedtype} para
%     especificar a natureza dessa relação. O campo \textit{reprinted-from} é
%     automaticamente convertido para \textit{related}; esse campo deve conter a
%     chave da obra relacionada. O campo \textit{relatedtype} pode conter
%     algumas
%     opções: \textit{relatedtype=\{reprintfrom\}} imprime ``Separata de''
%     (esse texto também é usado como padrão quando se escreve qualquer coisa
%     no campo \textit{reprinted-text}, usado no \textit{abntex2cite});
%     \textit{recensionof} imprime ``Recensão de''; \textit{reviewof} imprime
%     ``Resenha de''; \textit{reprintof} imprime ``Reimpressão de'';
%     \textit{translationof} imprime ``Tradução de''. Outras possibilidades
%     podem ser encontradas no arquivo \textit{brazilian.lbx}, incluso na
%     instalação padrão do \textit{biblatex}.
%   \item No \textit{biblatex}, as entradas \textit{inbook} e
%     \textit{incollection} não se comportam como no \textit{bibtex}. Entradas
%     \textit{inbook} também podem ter um \textit{bookauthor}; elas estão para
%     as entradas \textit{book} assim como as entradas \textit{incollection}
%     estão para as entradas \textit{collection} (Cf. seção 2.3.1 do manual
%     do \textit{biblatex}). Para que o autor da obra principal seja
%     substituído pelos traços sublineares, é necessário que o campo
%     \textit{bookauthor} seja igual ao campo \textit{author}.
%   \item É possível usar o campo \textit{furtherresp} como no
%     \textit{abntex2cite}, mas é preferível usar os campos \textit{editora},
%     \textit{editoratype}, \textit{editorb}, \textit{editorbtype} etc. (e.g.
%     \verb"editora = {Ismael Cardim},  editoratype={coeditor}" imprimirá
%     ``Co-edição de Ismael Cardim'' na biblioqgrafia; cf. entradas
%     \textit{hou\-a\-iss\-1996}, \textit{koogan1998}, \textit{ceravi1983},
%     \textit{riofilme1998} e pesquisar pelo campo \textit{editor\-a\-type} para
%     mais exemplos). Todas as opções para o campo \textit{editor\-type} podem
%     ser encontradas no arquivo \textit{abnt-brazilian.lbx}. Usar esses campos
%     em lugar do campo \textit{furtherresp} assegurará que as entradas sejam
%     impressas de forma consistente, embora os exemplos da própria ABNT não o
%     sejam.
%   \item Para tradutores pode-se usar o campo \textit{translator}.
%   \end{sloppypar}
% \end{itemize}
% % <<<2
